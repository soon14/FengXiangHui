...
- (void)createUI {
    ...
    // 消息列表
    self.chatTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, KMAINSIZE.width, KMAINSIZE.height-KBottomHeight-kInputViewHeight-_navHeight) style:UITableViewStylePlain];
    ...
    // 输入工具条
    self.chatInputView = [[QMChatRoomInputView alloc] initWithFrame:CGRectMake(0, KMAINSIZE.height-KBottomHeight-kInputViewHeight-_navHeight, KMAINSIZE.width, kInputViewHeight)];
    ...
    self.indicatorView.frame = CGRectMake((KMAINSIZE.width-150)/2, (KMAINSIZE.height-KBottomHeight-150-_navHeight-50)/2, 150, 150);
...
- (void)showGuestBookViewController {
    ...
    self.chatTableView.frame = CGRectMake(0, 0, KMAINSIZE.width, KMAINSIZE.height-KBottomHeight - _navHeight);
...
- (void)keyboardFrameChange: (NSNotification *)notification {
    ...
    if (newFrame.origin.y == [UIScreen mainScreen].bounds.size.height) {
        [UIView animateWithDuration:0.3 animations:^{
            self.chatInputView.frame = CGRectMake(0, KMAINSIZE.height-KBottomHeight-kInputViewHeight-_navHeight, KMAINSIZE.width, kInputViewHeight);
            self.chatTableView.frame = CGRectMake(0, 0, KMAINSIZE.width, KMAINSIZE.height-KBottomHeight-_navHeight-kInputViewHeight);
        }];
    }else {
        [UIView animateWithDuration:0.3 animations:^{
            self.chatInputView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height-kInputViewHeight-newFrame.size.height-_navHeight, KMAINSIZE.width, kInputViewHeight);
            self.chatTableView.frame = CGRectMake(0, 0, KMAINSIZE.width, [UIScreen mainScreen].bounds.size.height-_navHeight-kInputViewHeight-newFrame.size.height);
            [self scrollToEnd];
        }];
    }
}
