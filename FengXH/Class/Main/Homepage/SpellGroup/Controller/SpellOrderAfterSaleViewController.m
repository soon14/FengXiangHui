//
//  SpellOrderAfterSaleViewController.m
//  FengXH
//
...
@interface SpellOrderAfterSaleViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation SpellOrderAfterSaleViewController
...
-(void)AfterSaleRequest
{
    ...
    NSString * urlString = @"r=apply.groups.refund";
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if(indexPath.section==3)
    {
        ...
        if (_type==0) {
            cell.moneyTextField.text=_dataDic[@"refundprice"];
        }
        else
        {
            cell.moneyTextField.text=_dataDic[@"applyprice"];
    ...
    else
    {
        ...
        lab.text=[NSString stringWithFormat:@"*提示：您可退款的最大金额为¥%@",_dataDic[@"refundprice"]];
...
#pragma mark--------cheackImageView的Block(取消和删除)
-(void)cancelAction
{
...
-(void)addPictureWithImage:(UIImage *)image
{
    if(image!=nil){
        ...
        [_mImgNameArr insertObject:[NSString stringWithFormat:@"%@",[ShareManager getNowTimeTimestamp]] atIndex:_mImgNameArr.count];
...
#pragma mark-----提交审核
-(void)submitApplyWithImageStr:(NSString *)imageStr
{
    ...
    if ([price floatValue]>[_dataDic[@"refundprice"] floatValue]) {
        [DBHUD ShowInView:self.view withTitle:[NSString stringWithFormat:@"最大只能退款%@元",_dataDic[@"refundprice"]]];
        return;
    }
    ...
    NSString *tokenStr=[[NSUserDefaults standardUserDefaults] objectForKey:KUserToken];
    NSDictionary *paramsDic=@{@"token":tokenStr,@"orderid":_orderId,@"teamid":_teamId,@"price":price,@"rtype":@0,@"reason":reasonArr[reasonSelectIndex],@"content":contentStr,@"images":imageStr};
    NSString * urlString = @"r=apply.groups.refund.submit";
    NSString *path = [HBBaseAPI appendAPIurl:urlString];
    [DBHUD ShowProgressInview:self.view Withtitle:nil];
    [[HBNetWork sharedManager] requestWithMethod:POST WithPath:path WithParams:paramsDic WithSuccessBlock:^(NSDictionary *responseDic) {
        [DBHUD Hiden:YES fromView:self.view];
        if ([responseDic[@"status"] integerValue] == 1) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"applyAfterSaleSuccess" object:nil];
            [self.navigationController popViewControllerAnimated:YES];
        } else {
